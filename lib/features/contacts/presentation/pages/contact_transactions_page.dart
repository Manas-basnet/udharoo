...
class _ContactTransactionsPageState extends State<ContactTransactionsPage> {
  ...
  void _loadContactAndTransactions() {
    context.read<ContactCubit>().getContactByUserId(widget.contactUserId).then((
      contact,
    ) {
      if (mounted) {
        setState(() {
        ...
        body: BlocConsumer<ContactTransactionsCubit, ContactTransactionsState>(
          ...
          builder: (context, transactionState) {
            ...
            return RefreshIndicator(
              onRefresh: () async {
                context.read<ContactTransactionsCubit>().refreshTransactions(
                  widget.contactUserId,
                );
              },
              child: CustomScrollView(
                ...
                slivers: [
                  ...
                  if (transactionState is ContactTransactionsLoaded &&
                      transactionState.transactions.isNotEmpty && !_isMultiSelectMode)
                    _buildQuickStats(
                      theme,
                      horizontalPadding,
          ...
          child: SingleChildScrollView(
            ...
            child: Row(
              children: [
                ...
                _buildFilterChip(
                  'Completed',
                  ContactTransactionFilter.completed,
                  theme,
                  state,
                ),
              ],
  ...
  Widget _buildFilterChip(
    String label,
    ContactTransactionFilter filter,
    ThemeData theme,
    ContactTransactionsState state,
  ) {
  ...
  Widget _buildTransactionsSliver(
    ...
    if (state is ContactTransactionsError) {
      return SliverFillRemaining(child: _buildErrorState(state.message, theme));
    }

    final transactions = state is ContactTransactionsLoaded
        ? state.transactions
        : <Transaction>[];
  ...
  Widget _buildEmptyState(ThemeData theme) {
    ...
    switch (_selectedFilter) {
      ...
      case ContactTransactionFilter.completed:
        ...
        subtitle =
            'Completed transactions with ${_contact!.displayName} will appear here';
  ...
  void _createNewTransaction(BuildContext context) {
    context.go(Routes.transactionForm, extra: TransactionFormExtra(prefilledContact: _contact));
  }
